...
public class MyPerOpteryxJobImproved extends
SequentialBlackboardInteractingJob<MDSDBlackboard> implements
ICompositeJob {

    //DSECandidateConfiguration candidateConfig;

    public  MyPerOpteryxJobImproved(final DSEWorkflowConfiguration config, final DSELaunch launch) throws CoreException {
        super(false);
        
        // 1. Load PCM Models into memory
        this.addJob(new MyLoadPCMModelsIntoBlackboardJobImrpoved(config));

        // 2. Validate PCM Models
        this.addJob(new MyValidatePCMModelsJobImrpoved(config));
        ...
        this.add(new MyMoveInitialPCMModelPartitionJobImrpoved());

        // create the PCM partition anew but empty
        this.add(new MyPreparePCMBlackboardPartitionJobImrpoved());
        ...
        this.add(new MyOptimisationJobImrpoved(config, launch));

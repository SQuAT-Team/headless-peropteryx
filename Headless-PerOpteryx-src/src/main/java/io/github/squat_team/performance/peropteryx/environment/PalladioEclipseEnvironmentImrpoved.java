...
import io.github.squat_team.performance.peropteryx.environment.uriconverter.PrefixConverterImrpoved;
import io.github.squat_team.performance.peropteryx.environment.uriconverter.URIConverterHandlerImrpoved;
 ...
 */
public enum PalladioEclipseEnvironmentImrpoved {
    INSTANCE;

    private static final Logger log = Logger.getLogger(PalladioEclipseEnvironmentImrpoved.class.getName());

    private String config;
    private boolean isSetup = false;
    private URIConverterHandlerImrpoved uriConverter;

    PalladioEclipseEnvironmentImrpoved() {
    }
     ...
     */
    public synchronized void setup(String config) {
        if (this.isSetup) {
            return;
        }
        this.isSetup = true;
    ...
    private void registerUriConverter() {
        // create converter
        this.uriConverter = new URIConverterHandlerImrpoved(URIConverter.INSTANCE);
     ...
     */
    public URIConverterHandlerImrpoved getUriConverter() {
        assert this.isSetup();
    ...
    private void registerPathmapConverters() {
        ...
        this.uriConverter.addInterceptor(new PrefixConverterImrpoved("pathmap://PCM_MODELS/", this.config));
    }

...
import io.github.squat_team.performance.peropteryx.overwrite.MyLqnXmlHandlerImrpoved;

public abstract class MyAbstractLQNAnalysisImproved extends MyAbstractAnalysisImrpoved implements IAnalysis {
	...
	// cache for LQN models so that they do not have to be loaded so often.
	// Limited capacity (below) to not store too many models.
	private LinkedList<LqnModelType> recentModels = new LinkedList<LqnModelType>();
	private static int RECENT_MODEL_CAPACITY = 10;

	public MyAbstractLQNAnalysisImproved() {
		super(new MyLQNQualityAttributeDeclarationImrpoved());
	}
	...
	public IAnalysisResult retrieveResultsFor(PCMPhenotype pheno, Criterion criterion) throws AnalysisFailedException {
		...
		// NOTE: Added for Headless PerOpteryx to persist PCMs
		PCMFileExporterImrpoved.getInstance().savePCMChecked(pheno, pcm);
	...
	ILQNResult retrieveLQNSolverResults(PCMPhenotype pheno, PCMInstance pcm, Criterion criterion)
		...
		for (LqnModelType recentModel : this.recentModels) {
			if (isFileNameBelongsToModel(resultFileName, recentModel)) {
				model = recentModel;
			}
		}
		...
		if (model == null) {
			...
			model = MyLqnXmlHandlerImrpoved.loadModelFromXMI(resultFileName);
		}

		if (model == null) {
			logger.error(
					"LQN model " + resultFileName + " could not be loaded. See previous logging entries for details.");
			return new LQNNotConvergedResult(pcm);
		}

		// delete the oldest model from the recent model list; and add the
		// current one.
	...
	protected abstract ILQNResult retrieveResult(PCMInstance pcm, LqnModelType model, Criterion criterion)
			throws AnalysisFailedException;
	...
	private void launchLQNSolver(PCMPhenotype pheno, IProgressMonitor monitor)
		...
		builder = new PCMSolverConfigurationBasedConfigBuilder(this.config, ILaunchManager.RUN_MODE);
		builder.fillConfiguration(solverConfiguration);
		solverConfiguration.setInteractive(false);

		// Create a new Analysis job
		MyRunPCMAnalysisJobImrpoved solverJob = new MyRunPCMAnalysisJobImrpoved(solverConfiguration);

		solverJob.setBlackboard(blackboard);
		SolverStrategy strategy = solverJob.getStrategy();
		if (strategy instanceof MyPcm2LqnStrategyImrpoved) {
			this.previousResultFileName.put(pheno.getNumericID(), ((MyPcm2LqnStrategyImrpoved) strategy).getFilenameResultXML());
		} else {
			throw new AnalysisFailedException(
					"PCM solver was not configured to use Pcm2Lqn. Please contact the developers. Strategy was: "
							+ strategy);
		}

		try {

			// TODO catch exceptions due to convergence problems and handle them
			// nicely. For example, set the response time to MAXINT or similar.
	 ...
	 * 
	 * @throws CoreException
	 * @see de.uka.ipd.sdq.dsexplore.analysis.IAnalysis#initialise(org.eclipse.debug.core.ILaunchConfiguration,
	...
	public void initialise(DSEWorkflowConfiguration configuration) throws CoreException {
		...
		// objectives.add(new
		// UsageScenarioBasedObjective(this.getQualityAttribute(),
		// Objective.Sign.MIN, usageScenario));
		// }
		// }
	...
	// public UsageScenarioBasedObjective
	// translateEvalAspectToObjective(EvaluationAspectWithContext aspect,
	// UsageScenario usageScenario){
	// //Make sure, the aspect IS an objective
	// try {
	// if(aspect.getDimension().getType().getRelationSemantics().getRelSem() ==
	// EnumRelationSemantics.DECREASING) {
	// return new UsageScenarioBasedObjective(this.getQualityAttribute(),
	// Objective.Sign.MIN, usageScenario);
	// } else {
	// //INCREASING
	// return new UsageScenarioBasedObjective(this.getQualityAttribute(),
	// Objective.Sign.MAX, usageScenario);
	// }
	// } catch (CoreException e) {
	// e.printStackTrace();
	// throw new RuntimeException("Could not get response time quality
	// attribute!");
	// }
	// }

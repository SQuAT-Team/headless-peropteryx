...
public class MyOptimisationJobImrpoved implements IJob, IBlackboardInteractingJob<MDSDBlackboard> {
	...
	public MyOptimisationJobImrpoved(DSEWorkflowConfiguration config, DSELaunch launch) {
	...
	public void execute(IProgressMonitor monitor) throws JobFailedException, UserCanceledException {
		...
		try {
			...
			if (this.dseConfig.isOptimise()) {// use predefined instances as
				...
				PCMResourceSetPartition originalModelPartition = (MyPCMResourceSetPartitionImrpoved) blackboard
				...
				new SequentialExecutionInjectorImrpoved(dseConfig, evaluators, blackboard, monitor).inject();

				Opt4JStarter.runOpt4JWithPopulation(this.dseConfig, monitor, genotypes, allCandidates,
						archiveCandidates);
				PCMResultsProviderImrpoved.getInstance().extractValues();; // NOTE: ADDED FOR HEADLESS PEROPTERYX 
			}
